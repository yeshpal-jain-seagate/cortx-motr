...
#include "dtms0_common.h"
#include "dtms0/dtms0.h"
#include "rpc/rpclib.h"             /* m0_rpc_server_ctx */
#include "ut/misc.h"                /* M0_UT_PATH */
#include "ut/ut.h"

extern struct m0_reqh_service_type m0_dtms0_service_type;
...
static char **server_argv_alloc(const char *server_ep_addr, int *argc)
{
	...
	int         n;
	char      **ret;
	char        ep[DTMS0_ADDR_LEN];
	const char *argv[] = { "dtms0_st",
        "-e", ep,
        "-G", ep,
...
int dtms0_server_start(struct dtms0_params *dtp, const char *saddr)
{
	int argc;

	M0_PRE(saddr != NULL && *saddr != '\0');

	dtp->dtp_slogfile = m0_strdup(DTMS0_SERVER_LOGFILE);
	M0_ASSERT(dtp->dtp_slogfile != NULL);

	M0_ALLOC_PTR(dtp->dtp_sctx);
	M0_ASSERT(dtp->dtp_sctx != NULL);
	*dtp->dtp_sctx = (struct m0_rpc_server_ctx){
		.rsx_xprts_nr         = IO_XPRT_NR,
		.rsx_argv             = server_argv_alloc(saddr, &argc),
		.rsx_argc             = argc,
		.rsx_xprts            = &dtp->dtp_xprt,
		.rsx_log_file_name    = dtp->dtp_slogfile
	};
	return m0_rpc_server_start(dtp->dtp_sctx);
}
...
static void dtms0_op_populate(struct dtms0_params *dtp, int index,
			     struct m0_dtms0_op **dtms0_fops)
{
	int			i;
	int			rc;
	struct m0_dtms0_op       *dtms0fop;

	struct m0_dtms0_dtxrec   *dtxrec;
	struct m0_dtms0_opdescr *opdescr;

	/* int dto_rc; */

	M0_ASSERT(dtp != NULL);
	M0_ASSERT(dtms0_fops != NULL);

	dtms0fop = dtms0_fops[index];
	dtxrec    = &dtms0fop->dto_opdata;
	opdescr    = &dtms0fop->dto_descr;
