				    ...
				    m0_bcount_t            *limit)
{
	...
	for (records_ok = 0;
	      ...
	      * credits for the records to be deleted.
	      */
	 ...
	 * m0_be_btree_clear_credit(). Here, all nodes credits are being
	 * adjusted respectively to the number or records used during deletion
	 * (records_ok). Statistically the number of node credits after such
	 * adjustment has to be reasonable. In general, the following relation
	 * is fair: deleted_nodes_nr / all_nodes_nr == records_ok / records_nr.
	 */
	if (records_ok > 0 && records_nr >= records_ok) {
		nodes_cred.tc_reg_nr   = nodes_cred.tc_reg_nr   * records_ok / records_nr;
		nodes_cred.tc_reg_size = nodes_cred.tc_reg_size * records_ok / records_nr;
	}
